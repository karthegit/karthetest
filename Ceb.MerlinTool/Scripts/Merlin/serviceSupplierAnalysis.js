...
var supplierScoreByKPIKendo = {
    ...
    columns: [
       ...
       {
           field: "KPIAverage", title: "Category Average", format: "{0:n2}", width: 140, attributes: { style: "text-align: center" }
       }
    ],
    noRecords: true,
    messages: {
...
function getDemographic(companyId, callback) {
    var ddElement = '', selElement;
    var filter = $("#ddl-demographic option:selected").text();
    $.ajax({
        url: config_data.WEBAPI_URL + "/api/v1/APILookup/GetDemographic?companyId=" + companyId + "&filterType=" + filter + "&categoryId=" + $("#ddl-category").val() + "&supplierId=" + $("#ddl-supplier").val(),
        type: 'Get',
...
function getSupplierScore(companyId) {
    ...
    $.ajax({
        url: config_data.WEBAPI_URL + "/api/v1/APISupplierAnalysis/GetSupplierScore?companyId=" + companyId + "&category=" + category + "&supplier=" + supplier + "&demographic=" + demographic + "&demographicOption=" + demographicOption,
        type: 'Get',
        success: function (response) {
            $("#page-spinner").hide();
            if (response.Status == 1) {
                ...
                generateChartGauge(response.supplierScore, 'supplieroverallScore');
            }
            else {
                failureAlert(response.Message);
            }
...
function getSupplierScoreByKPI(companyId) {
    ...
    $.ajax({
        url: config_data.WEBAPI_URL + "/api/v1/APISupplierAnalysis/GetSupplierKPIScore?companyId=" + companyId + "&category=" + category + "&supplier=" + supplier + "&demographic=" + demographic + "&demographicOption=" + demographicOption,
        type: 'Get',
        success: function (response) {
            $("#page-spinner").hide();
            if (response.Status == 1) {
...
function getSupplierScoreByMetric(companyId) {
    ...
    $.ajax({
        url: config_data.WEBAPI_URL + "/api/v1/APISupplierAnalysis/GetSupplierMetricScore?companyId=" + companyId + "&category=" + category + "&supplier=" + supplier + "&demographic=" + demographic + "&demographicOption=" + demographicOption,
        type: 'Get',
        success: function (response) {
            $("#page-spinner").hide();
            if (response.Status == 1) {
                ...
                objScoreByMetric = generateHorizontalBar(response, 'supplierMetric', false, config_data.SupplierAnalysis.metric.XAxisMetric, config_data.SupplierAnalysis.metric.YAxisMetric, true, false, 'Supplier Metric Score', "Supplier Rating", "Category Average");
            }
            else {
                failureAlert(response.Message);
            }
    ...
    if (type == 'excel') {
        switch (container) {
            case 'supplierOverallContainer':
                ...
                downloadName = "Supplier Score";
                break;
            case 'supplierMetricContainer':
                excelData = excelMetric;
                downloadName = config_data.SupplierAnalysisTitleByMetric;
                break;
        ...
        if (excelData == "[]")
            ...
            exportGrid(element, excelData, companyDetails, "", downloadName, 'Supplier Scorecard');

    }
    else
        exportChart(element, type, companyDetails, "SupplierAnalysis", 'Supplier Scorecard');
}
...
function exportGridData(element, type) {
    if (excelComments == "[]")
        ...
        exportGrid(element, excelComments, companyDetails, "", "Supplier Comments", '');
